...
export default async function Page() {
	...
	// Most read from foundation content
	const mostReadFoundation = latestFoundationContent.slice(0, 3)
	...
	return (
		<div className="container mx-auto px-4">
				...
				<div className="flex flex-col lg:flex-row gap-y-8 lg:gap-x-16 mt-6">
					...
					>
						<div className="relative">
							...
							<Image
								src={
									latestFoundationContent[0].image ||
									"/default-image.jpg"
								}
								width={500}
								height={500}
			...
			<div className="mt-12">
				<SectionTitle title="الأنشطة الأكاديمية والبحثية" />
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mt-4">
					{/* Conferences */}
					{conferences.length > 0 && (
						<div className="space-y-4">
							<h3 className="text-base md:text-lg font-semibold text-primary dark:text-Muharram_primary">
								المؤتمرات العلمية
							</h3>
							<div className="space-y-3">
								{conferences.slice(0, 2).map((post) => (
									<PostCard key={post.id} {...post} />
								))}
							</div>
						</div>
