...
function toCard(item: Journals, t: Journals["translations"][0]): CardData {
  return { id: item.id, title: t.title, authors: t.authors,
           publishedYear: item.publishedYear, badge: t.publicationVenue, pdfUrl: item.pdfUrl };
}

function toRow(item: Journals, t: Journals["translations"][0]): TableRow {
  return { id: item.id, title: t.title, authors: t.authors ?? [],
           publicationVenue: t.publicationVenue ?? "", language: t.language,
           publishedYear: item.publishedYear, pdfUrl: item.pdfUrl };
}
...
export default function JournalsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    const base = !term ? all : all.filter((item) =>
    ...
    if (sortBy === "year")  return [...base].sort((a, b) => parseInt(b.publishedYear) - parseInt(a.publishedYear));
    if (sortBy === "title") return [...base].sort((a, b) => (a.translations[0]?.title ?? "").localeCompare(b.translations[0]?.title ?? ""));
    return base;
  }, [search, sortBy, all]);
  ...
  const paginate   = (p: number) => { setPage(p); setHighlightId(null); window.scrollTo({ top: 0, behavior: "smooth" }); };
