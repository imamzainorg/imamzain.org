...
export type SearchResult = { video: YouTubeVideo; groupSlug: string }
...
export default function FilterBar({
	...
	onSelectResult: (video: YouTubeVideo, groupSlug: string) => void
}) {
	...
	return (
		<div className="flex flex-wrap items-center gap-2 mb-4">
			<select
				...
				className="border border-white/10 rounded-xl px-3 py-2 text-sm bg-transparent text-slate-300"
			>
			...
			<div className="flex-1 min-w-[160px] relative">
				...
				<input
					...
					autoComplete="off"
					className="w-full bg-white/5 border border-white/10 rounded-xl pr-9 pl-3 py-2 text-sm text-white placeholder:text-slate-500"
				/>

				{/* عائمة فوق المشغل (z-30) — تختفي تلقائيًا لما ينفضى صندوق البحث */}
				{showDropdown && (
					<div className="absolute z-30 top-full mt-2 left-0 right-0 max-h-96 overflow-y-auto rounded-xl border border-white/10 bg-gray-900 shadow-2xl">
						{results.length === 0 ? (
							...
							results.map(({ video, groupSlug }) => (
								<button
									key={video.slug}
									type="button"
									// mousedown مو click، حتى ينفذ قبل ما input يخسر focus ويسكر
									// القائمة (blur) قبل لا نلحگ نقرأ الضغطة
									onMouseDown={(e) => {
										e.preventDefault()
										onSelectResult(video, groupSlug)
									}}
