...
const SocialLinks = ({ className, size }: { className?: string; size: SizeProp }) => (
        ...
        <div className="bg-primary text-white rounded-t-[2rem]">
            ...
            <div className="container max-lg:hidden py-10    ">
                ...
                <div className="flex justify-between gap-20 max-md:p-10 py-8  ">
                    <div className="w-[200px]">
                        <Image src="/images/logo-vertical-white.svg" alt="Logo" width={250}
                               height={200}/>
                    </div>
                        ...
                        {links.map((section: LinkSection, i: number) => (
                            <div key={i} className={`${i === 0 ? "row-span-2" : ""}`}>
                                ...
                                <ul className="flex flex-col gap-3 mr-5">
                                    {section.sublinks.map((sublink: Sublink, j: number) => (
                                        <Link className="text-sm" key={j} href={sublink.href}>
                                            {sublink.label}
                                        </Link>
                                    ))}
                                </ul>
                ...
                <div className="flex justify-end  ">
                    <p className="text-sm text-white/40 mt-10">جميع الحقوق محفوظة لمؤسسة الإمام زين العابدين
                        (ع) &copy; 1824</p>
                </div>
            ...
            <div className="lg:hidden p-8">
                    ...
                    {links.map((section, i) => (
                        <AccordionItem
                            ...
                            <div className="flex flex-col gap-5 mr-5 pb-3 ">
                                {section.sublinks?.map((sublink: Sublink, j: number) => (
                                    <Link className={'text-sm text-white'} key={j}
                                          href={sublink.href}>{sublink.label}</Link>
                                ))}
                            </div>
                        </AccordionItem>
                    ))}
                ...
                <div className=" items-center w-full flex justify-center py-10">
                    <Image className={'w-[200px]'} src="/images/logo-horizontal-white.svg" alt="Logo" width={250}
                           height={200}/>
                </div>
                <p className="text-center text-sm text-white/40">جميع الحقوق محفوظة لمؤسسة الإمام زين العابدين
                    (ع) &copy; 1824</p>
            </div>
        </div>
    );
